...
public class DialogPlayer extends JDialog {
	...
	/**
	 * Campo de texto con el a�o de nacimiento del player.
	 */
	...
	public DialogPlayer(Team pTeam, Player pPlayer) {
		...
		if(pPlayer.hasCard()) {
			File archImagen = new File("./data/images/" + pTeam.getCountry() + "/" + pTeam.getYear
					() + "/" + pPlayer.getCard().getImageName());
			icono = archImagen.exists() ? new ImageIcon(archImagen.getAbsolutePath()) : PanelCard
					.LAMINA_DEFECTO;
		} else {
		...
		String finalPos = position.substring(0, 1).toUpperCase() + position.substring(1);
		
		txtPosition = new JTextField(finalPos);
