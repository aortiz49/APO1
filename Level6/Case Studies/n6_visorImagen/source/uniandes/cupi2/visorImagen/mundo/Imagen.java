...
public class Imagen {
	...
	/**
	 * Crea una imagen a partir de la ruta del archivo donde esta la imagen original en BMP. La
	 * imagen numera los píxeles desde la esquina superior izquierda <br>
	 * de la imagen con (0,0). La coordenada X ve de 0 hasta el ancho-1 y la coordenada Y va de 0
	 * a el alto-1 Si la imagen es de ancho mayor al ANCHO_MAXIMO <br>
	 ...
	 *
	 * @return El color del pixel de coordenadas (x,y) o null en caso de que las coordenadas
	 * sobrepasen los límites de la imagen.
	 */
	 ...
	 * El negativo se calcula cambiando cada componente RGB, tomando el valor absoluto de restarle
	 * al componente 255.
	 */
	...
	/**
	 * Binarización: Consiste en llevar cada pixel de una imagen a negro o blanco. Para ello se
	 * requiere un umbral: si el color del pixel está <br>
	...
	/**
	 * Pixelamiento: Consiste en dividir la imagen en pequeñas regiones de pixeles y para cada una
	 * de esas regiones cambiar el color de los pixeles al <br>
	 * color promedio de dicha región. En este ejemplo, la región se dimensiona con los divisores
	 * más pequeños del ancho y el alto de la imagen.
	 */
	...
	/**
	 * Escala de grises: Para ello promedia los componentes de cada pixel y crea un nuevo color
	 * donde cada componente (RGB) tiene el valor de dicho promedio.
	 */
	 ...
	 * @param pDimension   Dimensión de la matriz de convolución. Dimension es válido para el
			...
			for (int j = 0; j < ancho; j++) {
				...
				for (int k = -pDimension / 2; k <= pDimension / 2; k++) {
					for (int l = -pDimension / 2; l <= pDimension / 2; l++) {
						sumaRed += pConvolucion[k + pDimension / 2][l + pDimension / 2] *
							copiaBorde[i + k + pDimension / 2][j + l + pDimension / 2].getRed();
						sumaGreen += pConvolucion[k + pDimension / 2][l + pDimension / 2] *
							copiaBorde[i + k + pDimension / 2][j + l + pDimension / 2].getGreen();
						sumaBlue += pConvolucion[k + pDimension / 2][l + pDimension / 2] *
							copiaBorde[i + k + pDimension / 2][j + l + pDimension / 2].getBlue();
	...
	/**
	 * Busca el color promedio de la región de la imagen El color promedio es formado por los
	 * promedios de rojos, verdes y azules de cada pixel.
	 *
	 ...
	 * @param pYFinal   Coordenada y del pixel final.
	 */
	private void cambiarColorRegion(Color pColor, int pXInicial, int pYInicial, int pXFinal, int
		pYFinal) {
	...
	/**
	 * Crea una copia de la imagen pero le adiciona un borde de pixeles de color negro, esto con
	 * el fin de poder operar con más facilidad la matriz de <br>
	...
	public int quantityPixelColors(Color pColor) {
		int quantity = 0;
		
		for (int i = 0; i < bitmap.length; i++) {
			for (int j = 0; j < bitmap[i].length; j++) {
				if (bitmap[i][j].getRed() == pColor.getRed()
					&& bitmap[i][j].getGreen() == pColor.getGreen()
					&& bitmap[i][j].getBlue() == pColor.getBlue())
					quantity++;
			}
		}
		return quantity;
	...
	public void printPixelRGB() throws InterruptedException {
		int red = 0;
		int green = 0;
		int blue = 0;
		...
		for (int i = 0; i < bitmap.length; i++) {
			for (int j = 0; j < bitmap[i].length; j++) {
				...
				System.out.println("(" + bitmap[i][j].getRed() + ", " + bitmap[i][j].getGreen() +
					" " +
					", " + bitmap[i][j].getBlue() + ")");
	...
	public int countGreen(int numRow) {
		int greens = 0;
		for (int j = 0; j < bitmap[numRow].length; j++) {
			if (bitmap[numRow][j].equals(Color.GREEN))
				greens++;
		}
		return greens;
	}
	...
	public String metodo1() {
		return Integer.toString(quantityPixelColors(new Color(255, 125, 255)));
	}
